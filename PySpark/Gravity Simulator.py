...
logging.info(f"Master URL: {spark.sparkContext.master}")
...
for [i,
  ...
  for planet_2 in planets_data[i + 1:]:
    gravity_results.append((planet_1[0], planet_2[0], calculate_gravity(planet_1,
                                                                      planet_2)))

logging.info("Gravity Calculation Results:")

for result in gravity_results:
  logging.info(f"Gravity between {result[0]} and {result[1]}: {result[2]:.3e} N")

end_time = time.time()
time_taken = end_time - start_time

logging.info(f"Time taken for gravity simulation: {time_taken:.2f} seconds")

results_df = spark.createDataFrame(gravity_results, ["Planet_1",
                                                     "Planet_2",
                                                     "Gravitational_Force"])
